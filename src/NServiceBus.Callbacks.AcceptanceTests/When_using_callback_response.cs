...
namespace NServiceBus.AcceptanceTests.Callbacks
{
    ...
    public class When_using_callback_response : NServiceBusAcceptanceTest
    {
        [Test]
        public void Should_send_back_old_style_control_message()
        {
            ...
            Scenario.Define(context)
                .WithEndpoint<EndpointWithLocalCallback>(b => b.Given(async (bus, c) =>
                    {
                        var response = bus.RequestWithTransientlyHandledResponse<CallbackResponse<OldEnum>>(new MyRequest(), new SendOptions());
